...
describe('Navigation Performance Tests', () => {
  ...
  describe('3. Entity Lookup Performance', () => {
    it('3.1 should find player in world of 1000+ rooms quickly', async () => {
    ...
    it('3.2 should handle object lookup in large inventory (1000+ items)', async () => {
      const player = await playerService.createPlayer({
        name: 'HoarderPlayer',
      ...
      for (let i = 0; i < 1000; i++) {
        ...
        objects.push(obj);
        await playerService.addInventoryItem(player.id, obj);
      }
    ...
    it('3.4 should perform spatial queries (find entities within radius)', async () => {
    ...
    it('3.5 should test entity indexing with hash map lookup', async () => {
  ...
  describe('4. Navigation Scaling', () => {
    ...
    it('4.8 should test navigation with player-to-room assignments', async () => {
      ...
      for (let i = 0; i < players.length; i++) {
        ...
        await roomService.addPlayerToRoom(targetRoom.id, players[i].id);
      }
      ...
      rooms.forEach((room) => {
        ...
        totalPlayersInRooms += (updatedRoom?.players.length || 0);
      });
  ...
  describe('5. World State Size Limits', () => {
    ...
    it('5.2 should handle maximum objects per room (1,000 objects)', async () => {
      ...
      for (let i = 0; i < 1000; i++) {
        ...
        await roomService.addObjectToRoom(room.id, obj.id);
      }
      const elapsed = performance.now() - start;

      const updatedRoom = roomService.getRoom(room.id);
      expect(updatedRoom?.objects.length).toBe(1000);
      console.log(`Adding 1000 objects to room took: ${elapsed.toFixed(2)}ms`);
      expect(elapsed).toBeLessThan(5000); // < 5 seconds
    });

    it('5.3 should handle maximum players per room (100 players)', async () => {
      const room = await roomService.createRoom({
        name: 'Crowded Room',
      ...
      for (let i = 0; i < 100; i++) {
        ...
        await roomService.addPlayerToRoom(room.id, player.id);
      }
      const elapsed = performance.now() - start;

      const updatedRoom = roomService.getRoom(room.id);
      expect(updatedRoom?.players.length).toBe(100);
